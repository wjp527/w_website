        ...
        <div className="mt-16 md:mt-28 grid grid-cols-1 md:grid-cols-2 gap-8">
          <div className="flex flex-col gap-8">
            {lastestPosts.map((item, index) => (
              <Card key={index} color={getPostColorFromCategory(item.data.category)} className={twMerge('max-w-xs md:max-w-md', (index === 1 || index === 3) && 'md:hidden')} buttonText="Read more">
          ...
          <div className="hidden md:flex flex-col gap-8 md:mt-10">
            {lastestPosts.map((item, index) => (
              <Card key={index} color={getPostColorFromCategory(item.data.category)} className={twMerge('max-w-xs md:max-w-md', (index === 0 || index === 2) && 'md:hidden')} buttonText="Read more">
                <Tag color={getPostColorFromCategory(item.data.category)}>{item.data.category}</Tag>
                <h3 className="font-heading font-black text-3xl mt-3">{item.data.title}</h3>
                <p className="text-lg text-zinc-400 mt-6">{item.data.description}</p>
              </Card>
